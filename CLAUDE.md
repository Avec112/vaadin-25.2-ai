...
Spring Boot 4.1.1 + Vaadin 25.2.8 (Flow, server-side Java UI) on Java 25, with Hibernate/JPA over an auto-configured in-memory H2 database (no entities are mapped yet). Generated from start.vaadin.com.
...
mvn test                             # all tests
mvn test -Dtest=KnowledgeChatViewTest   # one test class
mvn test -Dtest='KnowledgeChatViewTest#view_names_the_company_whose_documents_are_loaded'   # one test method
...
**Package-by-feature.** Each feature is a top-level package under `io.github.avec112` containing its domain classes (entity, repository, service), with UI either in the package itself (`ai/`) or in a nested `ui` subpackage (`rag/` + `rag/ui/`). `base/ui/` holds cross-cutting UI (`MainLayout`, `ViewTitle`). The `examplefeature` Task List scaffolding from start.vaadin.com has been deleted — do not reintroduce it.
...
**Encapsulation boundary.** Repositories and their `@Transactional` boundaries stay package-private; the `@Service` is the only public entry point; views take it via constructor injection. Views themselves are package-private classes (`KnowledgeChatView`). There are no JPA entities left after the Task List removal, but `spring-boot-starter-data-jpa` and H2 stay on the classpath for the first real persisted feature.
...
**Grid data access.** No view uses a `Grid` today. When one does, lazy-load through `setItems(query -> service.list(toSpringPageRequest(query)).stream())`, and have the repository return `Slice` rather than `Page` to avoid the extra count query.
